...
| 0x71C | `TX_REG_BPSK_CLOCK_SEL` | 圈内开闸点 `clock_sel`：0..count_max-1（450k 档 0..399，400k 档 0..449），时基走到 `time_sel` 那一圈、圈内第 `clock_sel` 拍才开读门。写 ≥ count_max 按"末拍"处理。**复位值 511**，所以不写它就是加这个寄存器之前的行为 |
...
`bpsk_time_sel` 是 **double**（单位 = 符号，可带小数），板子按当前速率把它拆成
`0x718` + `0x71C` 两个寄存器 —— 起点因此精确到 clk 而不是 1/Rs 的整数倍：
...
python tx_start.py --tsel 500.25              # 再往后四分之一个符号（clock_sel = 100/400）
python tx_start.py --tsel 500.0025             # 450k 档 1 个 clk = 1/400 符号（5.6 ns），寄存器的步进
...
**case 135 包格式**（`tx_start.py`，总长 18 字节）：
...
| 10..17 | `bpsk_time_sel` | double（8B，小端 IEEE-754），0 ≤ x < 1024，**单位 = 符号，可带小数**，起始位置，见下面 `time_sel` 一节 |

> 长度检查是 `len < 18` 就丢包（`printf` 报出来）—— 这个命令没有老包要兼容（字段从
> u16 改成 double 是**不兼容改动**，老脚本发的 12 字节包一律不认），所以宁可直接丢，也不猜
> 一个错的起点出来。范围外的**值**则相反：钳到 `[0, 1024)`，只 `printf` 提醒、不丢包 ——
> 长度对了就是一条有效命令，值不合法也别空发。上位机那一侧 `--tsel` 越界直接
> `SystemExit`，包都构造不出来。
...
（< 18 字节）；`--tsel` 超出 0 ≤ x < 1024 脚本直接报错，板上则钳到 1024（长度对就认，值
不合法也不空发）。case 136 只有命令字节、不看长度，短包也执行。case 133 仍然按长度补默认值
...
> `0x71C`（第 4 步），`tx_start()` 写的是 `tx_bpsk_time_sel` / `tx_bpsk_clock_sel` 这两个
> 全局量，不再是写死的 0 —— 所以 `--tsel` 每包都按你给的值起播，不会"上一包的校准被冲掉"。
> 代价是**校准值不再跨包延续**：不写 `--tsel` 就是 0（尽快开始），想要 500 就得每次都带上。
> 细节见下面 `time_sel` 一节。
>
> **小数部分是按 `tx_rate_sel` 换算的**：case 135 解析时读的是**当前**速率，把它拆成
> `time_sel` + `clock_sel`。这个换算正是为了"`--tsel 500.25` 在两种速率下都是四分之一
> 个符号"—— 450k 档一个符号 400 clk，拆出 `0x71C = 100`；400k 档一个符号 450 clk，
> 同样的小数拆出 `0x71C = 113`。两个都是 0.25 个符号 = 625 ns，**同一段真实时间**。
> 代价是 `clock_sel` 是"圈内第几拍"、会随速率变：所以**换速率之后要重发一次 case 135**，
> 板子才会按新速率重算。只 `--rate` 不重发的话 `0x71C` 还是老值，在 400k 档下
> `0x71C = 100` 只等于 100/450 个符号 —— 位置就偏了。
...
4. 写 0x718 + 0x71C      = 起始位置：time_sel（0..1023，整符号）+ clock_sel（圈内第几拍）
5. 脉冲 0x700：写 0 再写 1   ← 清读指针 rptr、符号计数器、done、时基计数器
6. 写 0x702 = 1          ← 开始发（等到时基走到 time_sel 那一圈的第 clock_sel 拍）；单次发发满 sym_num 个符号后
...
包里的字段在进 `tx_start()` 之前就解好了（`adhocSoft.c` 的 `tx_bpsk_single_shot` /
`tx_bpsk_sym_num` / `tx_bpsk_time_sel` / `tx_bpsk_clock_sel` 全局量 —— 符号数已经是换算完的
bit 数，`clock_sel` 也已经按 `tx_rate_sel` 从 `--tsel` 的小数部分换算完，和 `tx_rate_sel`
一个套路）。解码全在 case 135 里做：第 1、2..9、10..17 字节，其中第 10..17 那个 double
在那里被拆成 `tx_bpsk_time_sel`（整数部分）和 `tx_bpsk_clock_sel`（小数部分）。
第 5 步复位期间由 `set_bpsk_burst()`、`set_bpsk_time_sel()` 和 `set_bpsk_clock_sel()`
落到寄存器 —— 也就是第 2、3、4 步都是在复位窗口里做的，不会踩到半字更新，时基计数器这时
也被 hold 在 0，写什么值都在前面。

> **`time_sel` 和 `clock_sel` 必须成对写**。硬件是"时基走到 `time_sel` 那一圈、圈内第
> `clock_sel` 拍"开闸，两个一起才决定一个起点；单独只改一个，另一个还是老值，开闸点就跑
> 到别处去了。`tx_start()` 里这两笔相邻，中间不插任何东西（见下面一节的"不要在复位和
> `0x702` 之间插别的事情"）。
...
#### 起始位置 `time_sel` + `clock_sel`（0x718 / 0x71C）

"从哪儿开始发"是**两级**定的，`rtl/bpsk_ram.v` 里两级计数器各管一半：

| 级 | 计数器 | 范围 | 一格是 | 寄存器 |
|----|--------|------|--------|--------|
| 粗 | `cnt_1024` | 0..1023，1024 个符号一圈 | 1 个符号（2.22 µs @450k） | `0x718` `time_sel` |
| 细 | `count` | 0..count_max-1（450k 档 400，400k 档 450） | 1 个 clk（5.56 ns） | `0x71C` `clock_sel` |

符号分频器推着 `count` 转，`count` 数满一圈才把 `cnt_1024` 加一，所以读门 `tx_en` 开在
**`cnt_1024 == time_sel` 那一圈的第 `clock_sel` 拍**，之后一直开着 —— **`rd_en`（`0x702`）
拉低就立刻关门**，和改动前一样是"停发"开关，所以暂停/恢复不用脉冲复位。

加 `clock_sel` 之前只有粗那一级，起点只能落在 1/Rs 的整数倍上（450k 档 2.22 µs、400k 档
2.5 µs 一格）；现在粗档选圈、细档选圈内第几拍，**读门**这一层就精确到 1 个 clk 了
（5.56 ns），寄存器步进比改动前细 400/450 倍。不过**射出来的包络**另有 50 个 clk 的栅格
限制，实际收益是 8 倍而不是 400 倍 —— 见下面"分辨率的真实上限"一节。

五点要注意：

1. **时基和读脉冲同一个源**：`cnt_1024` 就是被那根符号分频脉冲推着走的，所以匹配上的那
   一拍在开门时自己没读表 —— **实际发出的第一个符号落在时基位置 `time_sel + 1`、圈内偏移
   `clock_sel`**（`tx_en` 是寄存器，开闸那一拍 `flag` 看它还是 0）。从复位放开算起是第
   `time_sel + 2` 个符号（第 1 个脉冲开门，第 2 个脉冲读表）。所以上位机给的 `x`
   实际落在 `x + 1`，`tx_start.py` 的打印里也照这个说法写。
2. **窗口只有一拍宽、一圈才来一次**：`cnt_1024 == time_sel && count == clock_sel` 只成立
   一个 clk。写晚了（时基已经走过这个值）就要等下一圈 —— 最多白等 1024 个符号（450k 档
   2.276 ms）。`tx_start()` 走的是"复位期间写"这条捷径：复位放开时时基从 0 起，那时写什么
   值都在前面、不用等。运行中改寄存器没有脚本 —— 这是有意的，见第 5 点。
3. **`clock_sel ≥ count_max` 按"末拍"处理**（`rtl/bpsk_ram.v` 里钳成 `count_max-1`）。
   这条兜底是给**兼容性**用的：`0x71C` 的复位值是 **511**，`ps/top.vhd` 在 VIO 模式
   （上电默认）下也接全 1，两个都 ≥ count_max，于是不论 450k 还是 400k 都落到各自的
   `count_max-1` —— 正好等于加 `clock_sel` 之前"分频脉冲那一拍开闸"的行为。**一个复位值
   同时管住两种速率**，所以不写 `0x71C` 的固件、VIO 模式、以及改动前的任何脚本，行为
   逐 bit 不变。
4. **两个寄存器都没有配套的模式位**：VIO 侧没有对应的 `vio_tx` 端口，所以 `ps/top.vhd`
   里 `bpsk_time_sel_mux` 在 VIO 模式下恒 0、`bpsk_clock_sel_mux` 恒 511，两个合起来还是
   老行为；只有 PS 模式才吃 `0x718` / `0x71C` 的值。也正因为走的是手写顶层的 mux，
   **不需要重新生成 VIO IP，也没动 BD**。
5. **要换起点只能重跑 `tx_start.py`**：`0x718` / `0x71C` 在 `tx_start()` 的复位窗口里写
   （第 4 步），和 `sym_num` / `single_shot` 一样是**开播参数**。早先那条"停发 → 写 `0x718`
   → 重发、全程不复位"的运行时路径（原来那条独立的起始时基命令，现已删除）已经去掉：它
   不脉冲 `0x700`，靠 `count` / `cnt_1024` 不受 `rd_en` 影响来保住参考时基，代价是每次改完
   都要等时基转回来。现在统一走复位窗口 —— 起点靠复位的确定性给出（`cnt_1024` 被 hold 在
   0，写什么值都在前面、不用等），代价是参考时基也一起归零，起点变成"相对本次突发开头"
   的位置。
   **单次发也适用**：`rptr` / `sym_cnt` / `done` 都是 `if(!rst_n || ~rd_en)` 清的，所以把
   `0x702` 拉低再拉高就能从第 0 个符号重新发一遍，不必脉冲 `0x700` —— 但那一下**不会读新的
   `time_sel` / `clock_sel`**，起点（含圈内偏移）保持不变。

**`0x702` 拉低/拉高同样要等下一圈**：`rd_en` 一低，`tx_en` 立刻清 0（`0x702` 还是停发开关）；
但再拉高时因为窗口只有一拍宽，要等时基下一圈转回 `time_sel`、圈内再到 `clock_sel` 才重新
开门 —— 最多白等 1024 个符号（450k 档 2.276 ms）。这条路径**不重写那两个寄存器**，所以起点
还是上一次 `tx_start.py` 写进去的那个，停发/恢复不会把它挪走。

`tx_start()` 里写的是 `set_bpsk_time_sel(tx_bpsk_time_sel)` 和
`set_bpsk_clock_sel(tx_bpsk_clock_sel)` —— **就是这两个全局量**（早先只有 `time_sel` 写死
0，现在两个都按包里的值走）。所以每条 case 135 都按自己带的 `--tsel` 起播，`--tsel 500.25`
重发多少遍都是 500 + 1/4 符号；不写就是 0 —— `time_sel = 0`、`clock_sel = 0` 之外，**不写
`0x71C` 时它是复位的 511**（= 末拍 = 老行为），所以"只给 `time_sel` 不给小数"的包和改动前
逐 bit 一致。

#### 分辨率的真实上限：50 个 clk，不是 1 个 clk

寄存器能给到 1 个 clk，但**射出来的包络动不了那么细**。读门（`flag`）本身是精确到拍的，
可这个 bit 还要穿过 `upsamping_450k` / `upsamping_400k`，其中第一级 `zero_interpolator`
（`TIME_FACTOR=400`、`INTERPOLATION_FACTOR=8` → `COUNT_MAX=50`）拿到 `x_valid` 之后不是立刻
发，而是**等自己那个自由跑的 /50 计数器走到下一个 slot** 才发。实测（把 DUT 复位干净，在
复位后的第 T 个 clk 给一拍 `x_valid`，看 `y` 第一次非零出现在第几拍）：

| `x_valid` 落在复位后第 | 1..50 | 51..100 | 101..150 | … |
|---|---|---|---|---|
| `y` 第一次非零出现在第 | 50 | 100 | 150 | … |

也就是说 50 个不同的开闸位置喂进去，输出**完全一样**；k 落在哪个 50 拍窗口里，输出就都
落在那个窗口的末尾 slot 上。所以：

- 有效步进是 **50 个 clk ≈ 278 ns @180 MHz**，不是 5.56 ns；
- `--tsel` 的小数部分给的 400 / 450 个取值，落到 RF 上只有 **8 个**（450k 档，400/50 = 8）
  或 **9 个**（400k 档，450/50 = 9）不同的位置 —— 每一格盖住 50 个连续的 `clock_sel` 值；
- 相对改动前（450k 档 1 个符号 = 2.22 µs）是 **8 倍**提升，不是 400 倍那么夸张。
  `tx_start.py` 在 `clock_sel` 不是 50 的整数倍时会打一行提示，告诉你这个值实际落在
  50-clk 栅格的哪一格上。

> 想要真正的"1 个 clk 分辨"，得让 `zero_interpolator` 的 `/50` 计数器相位也可编程
> （把开闸点同时写进它的计数器初值）。那是另一处改动，这次没做 —— 现在这个改动只保证
> **读门**精确到拍，`bpsk_ram` 这一层的行为在测试台里是逐 clk 验证过的。

> **启动比改动前晚一个符号**（仅限老行为 `clock_sel` = 末拍的情形）：读门要花一拍"开门"，
> 而且符号分频器现在跟 `rst_n`（= `TX_REG_RESET`）跑、不再跟 `RAM_EN` 跑，所以从"写
> `0x702` = 1"到第一个符号出去，比改动前晚一个符号左右（450k 档 4.4 µs 上下），具体相位
> 取决于 `RESET` 和 `RAM_EN` 两次写的间隔。稳态周期（400 / 450 clk）、"循环发一直发"、
> 单次发的符号数都不受影响。`clock_sel` 越小，开闸点越靠前，这一段的延迟也越短（圈内
> 偏移直接减掉）。
> **不要在 `0x700` 写 1 和 `0x702` 写 1 之间插别的事情**（printf、延时、等别的任务）：开闸
> 窗口只有一拍宽，错过了就要等整整一圈（409600 clk ≈ 2.28 ms）—— 不会丢，只是白白晚这么久。
> `tx_start()` 里这两笔之间只有两个 `emc_write`，是安全的。
> **每一圈的开闸点都在复位后的第 `clock_sel` 个 clk**，所以余量是 `clock_sel` 个 clk：老的
> 末拍（`0x71C` 不写 = 511 → 钳到 399）有近 400 clk ≈ 2.2 µs 的余量，`--tsel 0` 也稳；
> 而 `clock_sel` 很小（比如 `--tsel 0.0025` 给到 1）时余量只有 1 个 clk，`0x702` 几乎肯定
> 赶不上第 0 圈，于是**多等一圈**。位置不受影响（还是 `time_sel + 1` 那一圈、圈内
> `clock_sel` 拍），只是晚 2.28 ms 出来。要余量就挑个够大的 `--tsel`：门要到第 `time_sel`
> 圈才开（450k 档 `time_sel` × 400 clk），`--tsel 500` 就是约 1.1 ms 的余量，那 400 clk 的
> 写入延迟完全淹没在里面，`clock_sel` 给多小都不碍事。
...
`clock_sel` 单独占了四个用例，`first_valid` 的期望值统一写成
`first_valid_at(tsel, per_sym, csel) = per_sym × (tsel + 1) + csel - 1`：

- **用例 M**（`clock_sel = 0`，`time_sel = 4`）：第一个符号落在 1999 clk，比同 `time_sel`
  的老行为（2398）整整早 399 clk（= 一个符号周期）；周期仍是 400 clk —— 挪开闸点**不会**
  把符号速率拉长或压短；
- **用例 N**（`clock_sel = 100` 对 `200`，`time_sel = 4`）：两次跑出来的第一个符号相差
  **恰好 100 clk**。这是这次改动唯一想证明的事 —— 起点在 clk 级线性可调，不再被
  `1/Rs` 卡住；
- **用例 O**（450k 档 `clock_sel = 511`）：钳到末拍后 `first_valid = 2398`，和用例 I 的老
  期望**逐 bit 相同**；
- **用例 P**（400k 档 `clock_sel = 511`）：同一个 511 在这里钳到 449 而不是 399，
  `first_valid = 2248`，和用例 J 相同 —— 证明"一个复位值管住两种速率"。

`tb_bpsk_rate.v` 在本机需要自己补两个 IP 桩（`cmpy_0` / `dds_compiler_0` 是 Vivado 工程
里生成的，不在本仓库），补上之后能跑，验到的是**速率本身**：`rate_sel = 0` 读脉冲周期
400 clk、`rate_sel = 1` 是 450 clk，`sig` 与选中的那条链逐 bit 相同、没选中的那条泄放到
0。`clock_sel` 那条兜底钳位依赖"按 `rate_sel` 选 `count_max`"，所以这两个周期值正好是它的
旁证。`tb_tx.v` 要全链路（含 `qpsk` + cmpy），本机同样得靠桩，且是 100 万拍的端到端
仿真，时间长，通常只在 Vivado 里跑。
