...
（135 < 10 字节、136 < 3 字节）。case 133 仍然按长度补默认值（≤ 35 字节 → `rate_sel` = 0），
35 字节的老包照发不误。
...
>
> `tx_init()` 里写的也是这个全局量，所以校准值不会被后来的 `tx_start.py` 冲掉。
...
要再发一次就重发一遍 case 135（或只重复 4–5 步）；改符号数就回到 2。**发完后再触发必须
...
5. **case 136 就是靠这一对开关做的，全程不复位**（`set_bpsk_time_sel_runtime()`）：
   ...
   等一圈才恢复发数。
   单次发例外：跑完 `done` 会锁住（`sym_cnt`/`done` 同样只由 `rst_n` 清），光靠 `0x702`
   开关不会让这一串重新开始 —— 单次发要重来还是得发 case 135，那里面会脉冲 `0x700`。

`tx_init()` 里写的是 `set_bpsk_time_sel(tx_bpsk_time_sel)`，也就是 case 136
（`tx_time_calibration.py --tsel`）存下的全局量；没发过 case 136 时是 0。这一位只由
`reset_128M` 清、**不随 `TX_REG_RESET` 清**，所以每次初始化都显式写一遍，才不会把上一次
运行留下的值接着用。0 = 复位放开后第一拍就开门，等于尽快开始 —— 和改动前的默认行为对
得上。
